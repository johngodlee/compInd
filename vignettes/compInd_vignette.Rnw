...
<<>>=
alemdag_list <- lapply(dat_neighb12, function(x) {
	unlist(lapply(x, function(y) {
		alemdag(y$nb_diam[1:4], y$nb_dist[1:4], unique(y$focal_diam))
	}))
})
@

To use the other functions listed above, just switch the function name and particular arguments out in place of \texttt{alemdag()}.
...
Consider the plot below, which shows the effect of a buffer of 10 m radius in causing under-estimations of competition calculated by \texttt{crowdInd()}:
...
# Create boxplot
neighb_boxplot <- ggplot() + 
	geom_boxplot(data = dat, aes(x = edge, y = n_neighb, colour = edge))
...
wrap_plots(neighb_plot, neighb_boxplot, rad_plot) + 
	plot_layout(ncol = 1)
@

Plot 1 shows variation in the number of neighbours for each individual in the plot, plot 2 visualises this as a box plot, and plot 3 illustrates the results of \texttt{edgeExclude()}. In plot three each individual is given a buffer of the radius of the competition zone (10 m). These radii are coloured according to whether they are identified for exclusion by \texttt{edgeExclude()}. The red squares indicate the boundaries of the plot, and the green square indicates the 10 m buffer than \texttt{edgeExclude()} uses to identify points at the edge of the plot.
...
stopifnot(all(unlist(lapply(ab_list, nrow)) == ab_vec))

ggplot() + 
  geom_point(data = do.call(rbind, ab_list[1:20]), 
...
<<>>=
ab_neighb12 <- mclapply(ab_list, function(x) {
  neighb <- nearNeighb(x$x_grid, x$y_grid, x$stem_id, k = 12)
  out <- lapply(neighb, function(y) {
    left_join(y, x, by = c("focal" = "stem_id")) %>%
      rename(focal_diam = diam, focal_species = species, 
...
<<>>=
ab_summ <- do.call(rbind, mclapply(seq_along(ab_neighb12), function(x) {
  ...
  ab_spatialMingling_mean <- mean(spatialMingling(ab_list[[x]]$x_grid, ab_list[[x]]$y_grid, ab_list[[x]]$species))
  ab_winkelmass_mean <- mean(winkelmass(ab_list[[x]]$x_grid, ab_list[[x]]$y_grid, 4))

  out <- do.call(rbind, lapply(ab_neighb12[[x]], function(y) {
    data.frame(
      ab_alemdag = alemdag(y$nb_diam[1:4], y$nb_dist[1:4], unique(y$focal_diam)),
      ab_baLarger = baLarger(y$nb_ba[1:4], unique(y$focal_ba)),
      ab_baLocal = baLocal(y$nb_ba[1:4]),
      ab_crowdInd = crowdInd(y$nb_diam[1:4]),
      ab_dbhCorr = dbhCorr(y$nb_diam[1:4], unique(y$focal_diam)),
      ab_dbhDiff = dbhDiff(y$nb_diam[1:4], unique(y$focal_diam)),
      ab_dbhDom = dbhDom(y$nb_diam[1:4], unique(y$focal_diam)),
      ab_hegyi = hegyi(y$nb_diam[1:12], y$nb_dist[1:12], unique(y$focal_diam)),
      ab_lorimerComp = lorimerComp(y$nb_diam, y$nb_dist, unique(y$focal_diam), ab_lorimerCZR),
      ab_martinEk = martinEk(y$nb_diam[1:4], y$nb_dist[1:4], unique(y$focal_diam)),
      ab_pointDens = pointDens(y$nb_diam[1:4], y$nb_dist[1:4])
    )
  }))
...
The number of neighbours considered by \texttt{nearNeighb()} also affects the indices.
...
<<>>=
# Winkelmass with increasing spatial randomness
...
pdf(file = "wi_diagram.pdf", width = 8, height = 5)
wi_plot + wi_map_plot + 
