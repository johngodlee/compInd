...
\texttt{edgeExclude()} can be used to find individuals that are sufficiently distant from the edge of a plot. For competition indices which rely on a radius of expected neighbourhood effects such as \texttt{lorimerComp()}, \texttt{alemdag()} etc, individuals near the edges of the plot will have an erroneous under-estimation of competition compared to other individuals in the plot interior, because there is a lack of data outside the plot. It is common practice to exclude focal individuals if they are located a distance from the plot edge that is less than the radius of the competition zone. These individuals could still be used as competitors for other focal individuals, but not as focal individuals themselves. 
...
ab_summ <- do.call(rbind, mclapply(seq_along(ab_neighb), function(x) {
  ...
  ab_clarkEvans <- clarkEvans(ab_list[[x]][,c("x", "y")], 10000)
  ab_pielou_mean <- mean(pielou(ab_list[[x]][,c("x", "y")], 0, 100, 0, 100, 20))
  ab_spatialMingling_mean <- mean(spatialMingling(
  		  ab_list[[x]][,c("x", "y")], ab_list[[x]]$species))
  ab_winkelmass_mean <- mean(winkelmass(ab_list[[x]][,c("x", "y")], 4))
...
k_summ <- do.call(rbind, mclapply(k_vec, function(x) {
  do.call(rbind, lapply(ab_neighb[seq(2,182,20)], function(y) {
